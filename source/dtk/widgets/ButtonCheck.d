...
const auto ButtonCheckProperties = cast(PropSetting[])[
    PropSetting("gs_w_d", "ButtonCheckIndicatorAlignment", "indicatorAlignment",
            "IndicatorAlignment", "ButtonCheckIndicatorAlignment.LeftTop"),
...
class ButtonCheck : Widget
{
    ...
    void recalcIndicatorAndChildPositions()
    {
        ...
        const auto indicatorSizeHalf = cast(typeof(indicatorSize)) indicatorSize / 2;
    ...
    override Image renderImage()
    {
        ...
        if (form is null)
        {
            throw new Exception(this.toString() ~ ".renderImage() can't get Form");
        }
