...
const auto TextEntryProperties = cast(PropSetting[])[
    ...
    PropSetting("gs_w_d", "GenVisibilityMapForSubitemsLayout", "layout_lines",
            "LayoutLines", "GenVisibilityMapForSubitemsLayout.verticalTopToBottomAlignLeft"),
    PropSetting("gs_w_d", "GenVisibilityMapForSubitemsLayout", "layout_line_chars",
            "LayoutChars", "GenVisibilityMapForSubitemsLayout.horizontalLeftToRightAlignTop"),
    PropSetting("gs_w_d", "bool", "draw_bewel_and_background",
            "DrawBewelAndBackground", "true"),
    PropSetting("gs_w_d", "Color", "bewel_background_color",
            "BewelBackgroundColor", q{Color(cast(ubyte[3])[255,255,255])}),
    PropSetting("gs_w_d", "bool", "multiline", "Multiline", "false"),
    PropSetting("gs_w_d", "bool", "virtual_wrap_by_char",
            "VirtualWrapByChar", "false"),
    PropSetting("gs_w_d", "bool", "virtual_wrap_by_word",
            "VirtualWrapByWord", "false"),
    ...
    PropSetting("gs_w_d", "bool", "view_resize_by_content",
            "ViewResizeByContent", "false"),
    PropSetting("gs_w_d", "bool", "entry_resize_by_content",
            "EntryResizeByContent", "false"),
...
class TextEntry : Widget
{
    ...
    this()
    {
...

        {
            ...
            text_view.viewResized = delegate void() {
                ...
                if (getEntryResizeByContent())
                {
                    ...
                    debug writeln("TextEntry: set des size %sx%s".format(getWidth(), getHeight()));
                }
            };
        }
        ...
        static foreach (v; [
                stname("FontFamily", "string"), stname("FontStyle", "string"),
                stname("FontColor", "Color"), stname("FontSize", "ushort"),
                stname("FontItalic", "bool"), stname("FontBold", "bool"),
                ...
                stname("Multiline", "bool"), stname("VirtualWrapByChar", "bool"),
                stname("VirtualWrapByWord", "bool"),
                stname("ForceMonowidth", "bool"), stname("TextSelectable", "bool"),
                stname("TextEditable", "bool"), stname("CursorEnabled", "bool"),
    ...
    DrawingSurfaceI getDrawingSurfaceForTextView()
    {
        ...
        {
            ds = new DrawingSurfaceShift(ds, cast(int) padding_left, cast(int) padding_top);
        }

        return ds;
    }

    void on_keyboard_internal(string type, // TODO: better type for parameter
            EventKeyboard* event, int x, int y)
    {
    ...
    {
        text_view.click(event.mouseFocusedWidgetX + padding_left,
                event.mouseFocusedWidgetY + padding_top);
    }

    override void intMouseMove(Widget widget, EventForm* event)
    {
        on_keyboard_internal("up", event.event.ek,
                event.mouseFocusedWidgetX + padding_left, event.mouseFocusedWidgetY + padding_top);
    }

    override void intKeyboardPress(Widget widget, EventForm* event)
    {
        on_keyboard_internal("down", event.event.ek,
                event.mouseFocusedWidgetX + padding_left, event.mouseFocusedWidgetY + padding_top);
    }
    ...
    override void propagatePerformLayout()
    {
        ...
        else
        {
            ...
            debug writeln("TextEntry: text_view resized to: %sx%s".format(text_view.getWidth(),
                    text_view.getHeight()));
        }
