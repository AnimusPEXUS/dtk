...
const auto ScrollBarProperties = cast(PropSetting[])[
    ...
    PropSetting("gs_w_d", "Orientation", "orientation", "Orientation", q{Orientation.horizontal}),
...
class ScrollBar : Widget
{
    ...
    private void recalcScopeSpaceAndBewelSize()
    {
        ...
        scopeBewelSize = cast(int)(cast(float) scopeSpaceSize * getVisibleScope());
    ...
    private void recalcIndicatorAndChildrenPositions()
    {
        ...
        if (getOrientation() == Orientation.horizontal)
        {
            ...
            scopeBewelX = cast(int)(cast(float) buttonSize + (scopeFreeSpaceSize * getValue()));
            scopeBewelY = 0;
        ...
        else
        {
            ...
            scopeBewelX = 0;
            scopeBewelY = cast(int)(cast(float) buttonSize + (scopeFreeSpaceSize * getValue()));
    ...
    Tuple!(int, int, int, int) calcScrollingIntegers()
    {
        ...
        sMin = cast(int)(min + ((cast(float) max - min) * getValue()));

        sMax = cast(int)(sMin + ((cast(float) max - min) * getVisibleScope()));
    ...
    override void intMousePress(Widget widget, EventForm* event)
    {
        ...
        if (event.mouseFocusedWidgetX >= scopeBewelX && event.mouseFocusedWidgetX < scopeBewelX + scopeBewelW
                ...
                && event.mouseFocusedWidgetY < scopeBewelY + scopeBewelH)
        {
            ...
            p.widgetInternalDraggingEventStart(this, event.event.mouseX, event.event.mouseY,
                    delegate EnumWidgetInternalDraggingEventEndReason(Event* e) {
                if (e.type == EventType.mouse && e.em.type == EventMouseType.button
    ...
    override void intInternalDraggingEventStart(Widget widget, int initX, int initY)
    {
        if (getOrientation() == Orientation.horizontal)
        {
