...
T2 recursionGuard(T2, T1...)(ref bool already_called, Mutex call_mutex,
        T2 delegate() already_started_call, T2 delegate(T1) target, T1 args)
{
...
T2 recursionGuard(T2, T1...)(ref bool already_called, Mutex call_mutex,
        T2 already_started_return, T2 delegate(T1) target, T1 args)
{
    return recursionGuard(already_called, call_mutex, delegate T2() {
...
unittest
{
...

    {
        ...
        int t1()
        {
            int x = recursionGuard(called, mtx, cast(int) 11, delegate int() {
