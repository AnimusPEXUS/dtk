...
void genVisibilityMapForSubitems(int max_width, int max_height, int x, int y, int width, int height,

        GenVisibilityMapForSubitemsLayout layout, int delegate() getSubitemCount,
        int delegate(int subitem_index) getSubitemWidth,
        int delegate(int subitem_index) getSubitemHeight,
        void delegate(int subitem_index, int target_x, int target_y, int x,
            int y, int width, int height, bool its_the_last_visible_item) genVisibilityMapForSubitem,
        )
{
    ...
    int first_visible_item_offset;

    int last_visible_item;
    bool last_visible_item_found;
    int last_visible_item_offset;

    int subitem_count = getSubitemCount();
    ...
    {
        int items_count = last_visible_item - first_visible_item + 1;

        int delegate(int i) calc_loop_target_x;
        int delegate(int i) calc_loop_target_y;
        int delegate(int i) calc_loop_source_x;
        int delegate(int i) calc_loop_source_y;
        int delegate(int i) calc_loop_source_width;
        int delegate(int i) calc_loop_source_height;
        ...
        default:
            throw new Exception("unknown layout");
        case GenVisibilityMapForSubitemsLayout.horizontalLeftToRightAlignTop:
            calc_loop_target_x = delegate int(int i) {
                if (i == 0)
                    return 0;
                int width;
                for (int j = 0; j != i; j++)
            ...
            calc_loop_target_y = delegate int(int i) { return 0; };

            calc_loop_source_x = delegate int(int i) {
                if (i == 0)
                    return first_visible_item_offset;
                return 0;
            };

            calc_loop_source_y = delegate int(int i) { return 0; };

            calc_loop_source_width = delegate int(int i) {
                if (i == 0)
                    return getSubitemWidth(first_visible_item + i) - first_visible_item_offset;
                return getSubitemWidth(first_visible_item + i);
            };

            calc_loop_source_height = delegate int(int i) {
                return getSubitemHeight(first_visible_item + i);
            };
            break;
        case GenVisibilityMapForSubitemsLayout.verticalTopToBottomAlignLeft:
            calc_loop_target_x = delegate int(int i) { return 0; };

            calc_loop_target_y = delegate int(int i) {
                if (i == 0)
                    return 0;
                int height;
                for (int j = 0; j != i; j++)
            ...
            calc_loop_source_x = delegate int(int i) { return 0; };

            calc_loop_source_y = delegate int(int i) {
                if (i == 0)
                    return first_visible_item_offset;
                return 0;
            };
        ...
        for (int i = 0; i != items_count; i++)
        {
            ...
            genVisibilityMapForSubitem(i, tx, ty, sx, sy, sw, sh, i == items_count - 1);
        }
    }
...
class TextChar
{
    ...
    int getWidth(TextView text_view)
    {
        return getState(text_view).resImg.width;
    }

    int getHeight(TextView text_view)
    {
    ...
    private Image genImage(TextView text_view)
    {
        ...
        for (int x = 0; x != state.glyph.bitmap.width; x++)
        {
            auto tx = x + bearing_x;
            for (int y = 0; y != state.glyph.bitmap.height; y++)
            {
    ...
    int calcLineColumnIndex()
    {
        auto textchars = parent_line.textchars;
        foreach (i, v; textchars)
        {
            if (v == this)
            {
                return cast(int) i;
    ...
    Tuple!(TextLine, int) calcLineAndColumnIndex()
    {
        auto textchars = parent_line.textchars;
        foreach (i, v; textchars)
        {
            if (v == this)
            {
                return tuple(parent_line, cast(int) i);
            }
        }
        throw new Exception("char not found");
    }

    int calcSublineIndex(TextView text_view)
    {
        auto line_state = parent_line.getState(text_view);
        auto this_subline = calcSubline(text_view);
        foreach (i, v; line_state.sublines)
        {
            if (v == this_subline)
            {
                return cast(int) i;
    ...
    int calcSublineColumnIndex(TextView text_view)
    {
...
class TextLineSubline
{
    ...
    void recalculateWidthAndHeight(TextView text_view)
    {
        ...
        {
        default:
            throw new Exception("parent_text.getLineCharsLayout ",
    ...
    void genVisibilityMap(int this_line_index, int this_subline_index,
            int this_line_done_chars_count, int sublines_target_x,
            int sublines_target_y, int x, int y, int width, int height,

            TextView text_view, WidgetVisibilityMap visibility_map)
    {

        auto state = getState(text_view);

        genVisibilityMapForSubitems(state.width, state.height, x, y, width,
                height, parent_line.parent_text.getLineCharsLayout(), delegate int() {
            return cast(int) state.textchars.length;
        }, delegate int(int subitem_index) {
            return state.textchars[subitem_index].getWidth(text_view);
        }, delegate int(int subitem_index) {
            return state.textchars[subitem_index].getHeight(text_view);
        }, delegate void(int subitem_index, int target_x, int target_y, int x, int y,
                int width, int height, bool its_the_last_visible_item) {
...
class TextLine
{
    ...
    void recreateSublines(TextView text_view)
    {
        ...
        else
        {
            ...
            if (text_view.getVirtualWrapBySpace() || text_view.getVirtualWrapByChar())
            {
                switch (line_chars_layout)
                {
                default:
                    throw new Exception("not supported parent_text.getLineCharsLayout ",
        ...
        foreach (tc; textchars)
        {
            ...
            if ((text_view.getVirtualWrapBySpace()
                    || text_view.getVirtualWrapByChar()) && required_size <= current_size + s)
    ...
    void recalculateWidthAndHeight(TextView text_view)
    {
        ...
        if (state.sublines.length != 0)
        {
            ...
            {
            default:
                throw new Exception("parent_text.getLinesLayout ",
    ...
    dstring getText(int start, int stop)
    {
        dstring ret;
    ...
    void genVisibilityMap(// this is passed deeper to TextChar and stored to visibility_map items
            int this_line_index, int lines_target_x, int lines_target_y, int x, int y,
            int width, int height, TextView text_view, WidgetVisibilityMap visibility_map)
    {
        auto state = getState(text_view);

        genVisibilityMapForSubitems(state.width, state.height, x, y, width,
                height, parent_text.getLinesLayout(), delegate int() {
            return cast(int) state.sublines.length;
        }, delegate int(int subitem_index) {
            ...
            return state.sublines[subitem_index].getState(text_view).height;
                ...
                int width, int height, bool its_the_last_visible_item) {
            int sum = 0;
            /* int width_or_height_offset=0; */
            foreach (v; state.sublines[0 .. subitem_index])
            {
                sum += v.getState(text_view).textchars.length;
            }

            state.sublines[subitem_index].genVisibilityMap(this_line_index, subitem_index, sum,
                lines_target_x + target_x, lines_target_y + target_y, x, y,
                width, height, text_view, visibility_map);
        });

    }

    int calcSublinesCount(TextView text_view)
    {
        auto state = getState(text_view);
        return cast(int) state.sublines.length;
    }
...
const auto TextProperties = cast(PropSetting[])[
    PropSetting("gs_w_d", "TextMarkupType", "textMarkupType",
            "TextMarkupType", "TextMarkupType.plain"),

    PropSetting("gs_w_d", "GenVisibilityMapForSubitemsLayout", "lines_layout",
            "LinesLayout", "GenVisibilityMapForSubitemsLayout.verticalTopToBottomAlignLeft"),
    PropSetting("gs_w_d", "GenVisibilityMapForSubitemsLayout", "line_chars_layout",
            "LineCharsLayout", "GenVisibilityMapForSubitemsLayout.horizontalLeftToRightAlignTop"),

    PropSetting("gs_w_d", "string", "faceFamily", "FaceFamily", q{"Go"}),
    PropSetting("gs_w_d", "string", "faceStyle", "FaceStyle", "\"Regular\""),
    PropSetting("gs_w_d", "int", "faceSize", "FaceSize", "20*64"),
    PropSetting("gs_w_d", "int", "faceResolution", "FaceResolution", "72"),
    ...
    PropSetting("gs_w_d", "bool", "defaultFGColorEnabled",
            "DefaultFGColorEnabled", "true"),
    PropSetting("gs_w_d", "Color", "defaultFGColor", "DefaultFGColor", q{Color(0)}),

    PropSetting("gs_w_d", "bool", "defaultBGColorEnabled",
            "DefaultBGColorEnabled", "true"),
    PropSetting("gs_w_d", "Color", "defaultBGColor", "DefaultBGColor",
...
class Text
{
    ...
    mixin mixin_installSignal!("LinesRecalcRequired", "signal_linesRecalcRequired", false,);
    mixin mixin_installSignal!("VisibilityMapRecalcRequired",
            "signal_visibilityMapRecalcRequired", false,);
    mixin mixin_installSignal!("ImageRegenRequired", "signal_imageRegenRequired", false,);
    ...
    this()
    {
        ...
        static foreach (v; [
                ...
                stname("FaceFamily", "string"), stname("FaceStyle", "string"),
                stname("FaceSize", "int"), stname("FaceResolution", "int"),

                stname("Bold", "bool"), stname("Italic", "bool"),
    ...
    void recalculateWidthAndHeight(TextView text_view)
    {
        ...
        if (lines.length != 0)
        {
            ...
            {
            default:
                throw new Exception("getLinesLayout ", to!string(getLinesLayout()));
            case GenVisibilityMapForSubitemsLayout.horizontalLeftToRightAlignTop:
            case GenVisibilityMapForSubitemsLayout.horizontalRightToLeftAlignTop:
                foreach (l; lines)
                {
    ...
    dstring getText(int start, int stop)
    {
        ...
        int first_text_textchar;
        int first_text_textchar_index;

        bool last_found = false;
        int last_text_textchar;
        int last_text_textchar_index;
        ...
        foreach (i, l; lines)
        {
            ...
            {
                first_found = true;
                first_text_textchar = cast(int) i;
        ...
        ret ~= lines[first_text_textchar].getText(first_text_textchar_index,
                cast(int) lines[first_text_textchar].textchars.length);

        if (last_text_textchar - first_text_textchar > 1)
        {
            for (int i = first_text_textchar + 1; i < last_text_textchar; i++)
            {
                ret ~= "\n" ~ lines[i].getText();
            }
        }
    ...
    void genVisibilityMap(int x, int y, int width, int height, TextView text_view,
            WidgetVisibilityMap visibility_map)
    {
        auto state = getState(text_view);

        genVisibilityMapForSubitems(state.width, state.height, x, y, width,
                height, getLinesLayout(), delegate int() {
            ...
            return lines[subitem_index].getState(text_view).height;
                ...
                int width, int height, bool its_the_last_visible_item) {
            lines[subitem_index].genVisibilityMap(subitem_index, target_x,
                target_y, x, y, width, height, text_view, visibility_map);
        });
    ...
    bool isEOL(int line, int column)
    {
    ...
    TextChar getTextLineChar(int line, int column)
    {
        return getTextLine(line).textchars[column];
    }

    bool isEOL(int line, int subline, int subline_column, TextView text_view)
    {
        return getTextLineSubline(line, subline, text_view).getState(text_view)
            .textchars.length == subline_column && getTextLine(line)
            .getState(text_view).sublines.length - 1 == subline;
    }

    bool isEOSL(int line, int subline, int subline_column, TextView text_view)
    {
        return getTextLineSubline(line, subline, text_view).getState(text_view)
            .textchars.length == subline_column;
    }

    TextLineSubline getTextLineSubline(int line, int subline, TextView text_view)
    {
        return getTextLine(line).getState(text_view).sublines[subline];
    }

    TextChar getTextLineSublineChar(int line, int subline, int column, TextView text_view)
    {
        return getTextLineSubline(line, subline, text_view).getState(text_view).textchars[column];
    }
...
const auto TextViewProperties = cast(PropSetting[])[
    ...
    PropSetting("gs_w_d", "int", "selection_start", "TextSelectionStart", "0"),
    PropSetting("gs_w_d", "int", "selection_end", "TextSelectionEnd", "0"),
    ...
    //PropSetting("gs_w_d", "int", "cursor_position_line", "CursorPositionLine", "0"),
    //PropSetting("gs_w_d", "int", "cursor_position_column", "CursorPositionColumn", "0"),

    PropSetting("gs_w_d", "bool", "readOnly", "ReadOnly", "false"),

    PropSetting("gs_w_d", "TextViewMode", "textViewMode", "TextViewMode",
            "TextViewMode.singleLine"),

    PropSetting("gs_w_d", "bool", "fixedSoftWrapSizeEnabled",
            "FixedSoftWrapSizeEnabled", "false"),
...
class TextView
{
    ...
    this()
    {
        ...
        static foreach (v; [
                stname("Width", "int"), stname("Height", "int"),
    ...
    void drawImageToDrawingSurface(int target_x, int target_y, int width,
            int height, DrawingSurfaceI ds)
    {
        if (ds is null)
            ds = getDrawingSurface();
    ...
    void ensureTimer500Connection()
    {
        ...
        {
            auto p = getPlatform();
            timer500_signal_connection = p.connectToSignal_Timer500(delegate void() nothrow{
                collectException({
    ...
    void setText(Text txt)
    {
        ...
        text_linesRecalcRequired_sc = text.connectToSignal_LinesRecalcRequired(
                delegate void() nothrow{ linesRecalcRequired = true; });
    }
    ...
    void drawWidgetVisibilityMapWidget(WidgetVisibilityMapWidget e, bool redrawOnImage,
            bool copyToDS)
    {
        ...
        if (redrawOnImage)
        {
            ...
            auto chr_image = chr_state.resImg.getImage(e.x, e.y, e.width, e.height);

            // auto tv_x = getX();
            // auto tv_y = getY();

            main_loop: for (int y = 0; y != chr_image.height; y++)
            {
                for (int x = 0; x != chr_image.width; x++)
                {
                    ...
                    ImageDot fg_dot;
                    {
                        ...
                        bg_dot = _rendered_image.getDot(t_x_x, t_y_y);
                    }
                    {
                        auto t_x_x = x;
                        auto t_y_y = y;
                        ...
                        fg_dot = chr_image.getDot(t_x_x, t_y_y);
                    }
                    if (fg_dot.enabled)
                    {
                        ...
                        _rendered_image.drawDot(Position2D(cast(int)(x + e.target_x),
                                cast(int)(y + e.target_y)), id);
                    }
                    else
                    {
                        _rendered_image.drawDot(Position2D(cast(int)(x + e.target_x),
                                cast(int)(y + e.target_y)), bg_dot);
                    }
                }
            }
        ...
        if (copyToDS)
            drawImageToDrawingSurface(e.target_x, e.target_y, e.width, e.height, null);
    }
    ...
    void genImage()
    {
...

        {
            ...
            for (int y = 0; y != h; y++)
            {
                for (int x = 0; x != w; x++)
                {
                    _rendered_image.drawDot(Position2D(cast(int) x, cast(int) y), bg_dot);
                }
            }
        }
    ...
    void clearCursor(int x, int y, int width, int height)
    {
        drawImageToDrawingSurface(x, y, width, height, null);
    }

    Tuple!(bool, int, int, int, int) calculateVisibleCursor(CursorPosition cursor_pos)
    {
        debug
        {
        ...
        auto fail_res = tuple(false, 0, 0, 0, 0);
    ...
    void changeCursorPositionByCoordinates(int x, int y)
    {
    ...
    Tuple!(WidgetVisibilityMapWidget, // 0
    ...
    {
        auto fail_res = tuple(cast(WidgetVisibilityMapWidget) null, cast(WidgetVisibilityMapWidgetClickLeanH) 0,
                cast(WidgetVisibilityMapWidgetClickLeanV) 0, cast(TextChar) null,
                cast(WidgetVisibilityMapWidget) null, cast(TextChar) null,
        ...
        foreach (v; visibility_map.elements)
        {
            if ((x >= v.target_x && x < v.target_x + v.width)
        ...
        return tuple(res_element, lh, lv, prev_char, prev_element, next_char, next_element,);
    }
    ...
    void textInput(dstring txt)
    {
        ...
        line.textchars = line.textchars[0 .. index] ~ new_chars ~ line.textchars[index .. $];
        ...
        if (event.keyState == EnumKeyboardKeyState.pressed)
        {
                ...
                if (cp.chr !is null)
                {
                    ...
                    if (line_column_index == 0)
                    {
                        ...
                        if (prev_line !is null)
                        {
                            ...
                            text.lines = text.lines[0 .. line_index] ~ text.lines[line_index
                                + 1 .. $];
            ...
            if (event.keysym.keycode == EnumKeyboardKeyCode.Delete)
            {
                ...
                else
                {
                    ...
                    int line_index = line.calcLineIndex();
    ...
    void cursorMoveToNextOrToPrevLine(bool prev)
    {
        ...
        int cursor_subline_index;
        int cursor_line_subline_count;

        TextLineSubline cursor_subline;
        int cursor_subline_column_index;
        ...
        if (cursor.chr !is null)
        {
            ...
            cursor_line_subline_count = cast(int) cursor_line.getState(this).sublines.length;
        ...
        int same_line_chars_width_or_height;
        ...
        {
            new_subline = cursor_line.getState(this)
                .sublines[cursor_subline_index + (prev ? -1 : +1)];
        }

        TextChar new_char;
        bool new_atEOL;
        bool new_atEOVL;
        int new_line_chars_width_or_height;
        ...
        foreach (v; new_subline.getState(this).textchars)
        {
            ...
            debug writeln("new_line_chars_width_or_height <= same_line_chars_width_or_height:",
    ...
    void cursorMoveToNextOrToPrevChar(bool prev)
    {
        ...
        {
            int chr_index = cp.chr.calcSublineColumnIndex(this);
            auto subline_textchars = subline.getState(this).textchars;
            if (!prev)
            {
